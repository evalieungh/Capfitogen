...
if(celdas){
  ...
  if(nrow(puntosfuera)>0){
    ...
    writeOGR(puntosfuera,dsn=paste(resultadosCell),layer="points_out_grid",driver="ESRI Shapefile")
  }
  puntosorig<-subset(puntosorig,!is.na(cellvalue))
  puntosorigshp<-SpatialPointsDataFrame(puntosorig[,c(6,5)],puntosorig)
  writeOGR(puntosorigshp,dsn=paste(resultadosCell),layer="final_analyzed_points",driver="ESRI Shapefile")
...
if(areas){
  ...
  if(WDPA){
    ...
    if(coveran){
      ...
      writeOGR(ttt,dsn=paste(resultadosCOVERAN),layer="CoverWDPAmap",driver="ESRI Shapefile")
    ...
    writeOGR(puntos,dsn=paste(resultadosWDPA),layer="final_analyzed_points",driver="ESRI Shapefile")
    writeOGR(aaa,dsn=paste(resultadosWDPA),layer="TotalWDPAmap",driver="ESRI Shapefile")
    writeOGR(bbb,dsn=paste(resultadosWDPA),layer="ComplementaryWDPAmap",driver="ESRI Shapefile")
...
if(areas){
  ...
  if(propio){
    ...
    setwd(paste(ruta))
    #ingreso ?rea protegida
    areasprot<-readOGR(paste(ruta,"/wdpa/",nombre,".shp",sep=""))
    ...
    if(coveran){
      ...
      rownames(areasWDPA)<-rownames(ttt@data)
      ttt<-spCbind(ttt,areasWDPA)
      writeOGR(ttt,dsn=paste(resultadosCOVERAN),layer="CoverAREASmap",driver="ESRI Shapefile")
      taxain<-data.frame(table(tablain$CHAIN))
      colnames(taxain)[1]<-"Taxa"
      colnames(taxain)[2]<-"Pops_Conserved"
      taxatotal<-data.frame(table(tablatotal$CHAIN))
    ...
    writeOGR(bbb,dsn=paste(resultadosWDPA),layer="ComplementAreas",driver="ESRI Shapefile")
    #An?lisis top-?reas
