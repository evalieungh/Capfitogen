#######################################################################
# Script  de la herramienta GEOQUAL 2.0 - GEOQUAL 2.0 Tool script 
...
setwd(paste(ruta))
write("GEOQUAL line 40 - Ok", file="Error/process_info.txt", append=TRUE)
...
if(system.file(package="mapview")==""){
  packages2<-append(packages2,"mapview")
}
...
library(googleVis)
library(mapview)
write("GEOQUAL line 93 - Ok", file="Error/process_info.txt", append=TRUE)
...
write("GEOQUAL line 119 - Ok", file="Error/process_info.txt", append=TRUE)
...
if(length(sexagesimal[,1])>0){
  ...
  write("GEOQUAL line 152 - Ok", file="Error/process_info.txt", append=TRUE)
  ###Unificacion coordenadas sexagesimal a decimal y decimal original
...
write("GEOQUAL line 160 - Ok", file="Error/process_info.txt", append=TRUE)

#Validacion de las coordenadas, si error, aviso
...
year<-substr(puntosorig[,31], 1,4)
GADM<-vector(mode="character",length=length(year))
GLC<-vector(mode="character",length=length(year))
#Asignacion de anhos y versiones GADM y GLC para cada accesion/observacion
for (i in 1:length(year)){
  if(is.na(year[i])){
    ...
    if(year[i]<=1996){
      ...
      if(year[i]>1996&year[i]<=2008){
        GADM[i]<-"version2_8"
        GLC[i]<-"glc2000"
      }else{
        if(year[i]>2008&year[i]<=2015){
          GADM[i]<-"version2_8"
          GLC[i]<-"glc2015"
        }else{
          if(year[i]>2015&year[i]<=2019){
            GADM[i]<-"version4_1"
            GLC[i]<-"glc2015"
          }else{
          if(year[i]>2019){
...
puntosorig<-cbind(puntosorig,year,GADM,GLC)


############################################################
############################################################
#Creacion de un archivo espacial de puntos
...
############################################################
#Si no hay informacion sobre el anho de recoleccion de ninguna accesion, se va por via GEOQUAL normal actualizado 2024
if(all(is.na(year))){
  ...
  if(length(enmar[,1]!=0)){
    ...
    #Determinaci?n paises en datos objetivo desde coord o localidad que tienen costas
    ...
    #Carga y extracci?n desde anillo 1km paises desde coord o local con mar
    aaa<-list()
  ...
  ##########
  #Importacion capa Global Land Cover 2000 ahora a 300 m
  ...
  write.table(pasaporte2, file=paste(resultados,"/PasaporteOriginalEvaluadoGEOQUAL.txt",sep=""), sep = "\t", row.names = FALSE, qmethod = "double", ,fileEncoding = "latin1")
  write.table(pasaporte2, file=paste(resultados,"/PasaporteOriginalEvaluadoGEOQUAL.xls",sep=""), sep = "\t", row.names = FALSE, qmethod = "double", ,fileEncoding = "latin1")
  ...
  rm(georefcol)
  write.table(unitabla, file=paste(resultados,"/tabla_de_analisisGEOQUAL.txt",sep=""), sep = "\t", row.names = FALSE, qmethod = "double",fileEncoding = "latin1")
  write.table(unitabla, file=paste(resultados,"/tabla_de_analisisGEOQUAL.xls",sep=""), sep = "\t", row.names = FALSE, qmethod = "double",fileEncoding = "latin1")
...
############################################################
#Si hay informacion sobre el anho de recoleccion de al menos una  accesion, se va por via GEOQUAL2
if(!all(is.na(year))){
  if(all(GADM=="version2_8")){
    #Carga de shapefiles GADM a nivel pais
    load("ID0.RData")
    if(proj4string(puntos)!=proj4string(ID0)){
      proj4string(ID0)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
      proj4string(puntos)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
    ...
    #Extraccion del ISO de pais para los puntos y carga de GADM de solo paises involucrados
    CTYcode<-over(puntos,ID0)
    CTYcode<-cbind(puntosorig[,c(2,25,23)],CTYcode)
    rm(ID0)
    write("9.Extracci?n del ISO de pa?s para los puntos y carga de GADM de s?lo pa?ses involucrados", file="Error/process_info.txt", append=TRUE)
  }else{
    #Carga GADM mundial 2010
    load("ID0.RData")
    if(proj4string(puntos)!=proj4string(ID0)){
      proj4string(ID0)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
      proj4string(puntos)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
    }
    puntos<-SpatialPointsDataFrame(puntosorig[,c(25,23)],puntosorig,proj4string=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
    puntos1<-subset(puntos,puntos@data$GADM=="version2_8")
    puntosorig1<-subset(puntosorig,GADM=="version2_8")
    puntosorig2<-subset(puntosorig,GADM=="version4_1")
    #Extraccion del ISO de pais para los puntos y carga de GADM de solo paises involucrados
    CTYcode1<-over(puntos1,ID0)
    CTYcode1<-CTYcode1[,1]
    CTYcode1<-cbind(puntosorig1[,c(2,25,23)],CTYcode1)
    colnames(CTYcode1)[ncol(CTYcode1)]<-"ISO"
    rm(ID0)
    #Carga GADM mundial 2022
    load("ID0_41.RData")
    puntos2<-subset(puntos,puntos@data$GADM=="version4_1")
    if(proj4string(puntos)!=proj4string(ID0)){
      proj4string(ID0)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
      proj4string(puntos2)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
    }
    #Extraccion del ISO de pais para los puntos y carga de GADM de solo paises involucrados
    CTYcode2<-over(puntos2,ID0)
    colnames(CTYcode2)[1]<-"ISO"
    CTYcode2<-cbind(puntosorig2[,c(2,25,23)],CTYcode2)
    rm(ID0)
    CTYcode<-rbind(CTYcode1,CTYcode2)
    rm(CTYcode1)
    rm(CTYcode2)
    rm(puntosorig1)
    rm(puntosorig2)
    rm(puntos1)
    rm(puntos2)
    i <- order(CTYcode$ACCENUMB)
    CTYcode <- CTYcode[i,]
  }
  
  #Separacion en puntos que caen en mar o aguas internas (no rios)
  enmar<-subset(CTYcode,is.na(ISO))
  if(length(enmar[,1])!=0){
    enmar1<-SpatialPoints(enmar[,c(2,3)],proj4string=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
  }
  ...
  if(!any(GADM=="version4_1")){
    aaa<-list()
    ISOcol<-which(names(CTYcode)=="ISO")
    CTY<-unique(CTYcode[,ISOcol])
    rm(ISOcol)
    for(i in 1:length(CTY)){
      pais1<-paste("CTY/",CTY[i],sep="")
      paisext<-st_read(paste(ruta,"/",pais1,".shp",sep=""))
  ...
  if(any(GADM=="version4_1")){
    #Parte antigua GADM 2.8
    CTY1<-cbind(CTYcode,GADM)
    CTY1<-subset(CTY1,GADM=="version2_8")
    aaa<-list()
    ISOcol<-which(names(CTYcode)=="ISO")
    CTY<-as.character(unique(CTY1[,ISOcol]))
    for(i in 1:length(CTY)){
      pais1<-paste("CTY/",CTY[i],sep="")
      paisext<-st_read(paste(ruta,"/",pais1,".shp",sep=""))
      paisext<-as_Spatial(paisext)
      proj4string(paisext)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"  
      puntosext<-subset(CTY1,ISO==paste(CTY[i]))
      puntosext1<-SpatialPoints(puntosext[,c(2,3)],proj4string=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
      if(proj4string(puntosext1)!=proj4string(paisext)){
        proj4string(paisext)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
        proj4string(puntosext1)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
    ...
    colnames(extraccion)[1]<-"ACCENUMB"
    rm(paisext)
    #Parte moderna GADM 4.1
    CTY2<-cbind(CTYcode,GADM)
    CTY2<-subset(CTY2,GADM=="version4_1")
    CTY<-as.character(unique(CTY2$ISO))
    rm(ISOcol)
    bbb<-list()
    for(i in 1:length(CTY)){
      load(paste("CTY41/",CTY[i],".RData",sep=""))
      proj4string(paisext)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"  
      puntosext<-subset(CTY2,ISO==paste(CTY[i]))
      puntosext1<-SpatialPoints(puntosext[,c(2,3)],proj4string=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
      if(proj4string(puntosext1)!=proj4string(paisext)){
        proj4string(paisext)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
        proj4string(puntosext1)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
      }
      bbb[[paste(CTY[i])]]<-cbind(puntosext[,1],over(puntosext1,paisext))
    }
    extraccion2<-do.call("rbind",bbb)
    colnames(extraccion2)[1]<-"ACCENUMB"
    extraccion<-rbind(extraccion,extraccion2)
    rm(CTY1)
    rm(CTY2)
    rm(extraccion2)
    rm(puntosext)
    rm(puntosext1)
    i <- order(extraccion$ACCENUMB)
    extraccion <- extraccion[i,]
  }
  
  write("11.Extraccion de informacion para puntos que caen en tierra", file="Error/process_info.txt", append=TRUE)
  ...
  if(length(enmar[,1])!=0){
    ...
    paismar<-read.delim("ListaPaisesMar.txt")
    CTY2<-as.character(unique(pasaporte$ORIGCTY))
    CTY2<-CTY2[!is.na(CTY2)]
    CTY2<-unique(c(as.character(CTY),CTY2))
    CTY2<-data.frame(cbind(1:length(CTY2),CTY2))
    colnames(CTY2)[2]<-"ISO"
    ...
    rm(bbb)
    rm(ccc)
    write("12.Extraccion de informacion para puntos que caen en mar_alta precision", file="Error/process_info.txt", append=TRUE)
  } 
  #Unificacion tablas pasaporte y tablas extraccion desde coordenadas
  ###################
  unitabla<-merge(puntosorig,extraccion,by="ACCENUMB",all.x=TRUE)
  rm(extraccion)
  if(length(enmar[,1])!=0){
  ...
  write("13.Unificacion tablas pasaporte y tablas extraccion desde coordenadas", file="Error/process_info.txt", append=TRUE)
  
  #SUITQUAL# -> A todo con coordenadas
  ##########

  if(any(GLC=="glc1992")){
    #Importacion capa Global Land Cover X 300 m para el anho en cuestion
    
    #Extraccion de uso de suelo en bruto
    unitabla1<-subset(unitabla,GLC=="glc1992")
    glc1992<-raster("CTY41/GLC/glc1992_300.tif")
    globlandc<-extract(glc1992,unitabla1[,c("DECLONGITUDE","DECLATITUDE")])
    unitabla1<-cbind(unitabla1,globlandc)
    rm(globlandc)
    rm(glc1992)
    
    #Extraccion de uso de suelo para cultivadas/silvestres ya interpretado
    unitabla1c<-subset(unitabla1,SAMPSTAT!=100&SAMPSTAT!=110&SAMPSTAT!=120|is.na(SAMPSTAT))
    unitabla1w<-subset(unitabla1,SAMPSTAT==100|SAMPSTAT==110|SAMPSTAT==120)
    if(length(unitabla1c[,1])!=0){
      glc1992c<-raster("CTY41/GLC/glc1992_c.tif")
      SUITQUAL<-extract(glc1992c,unitabla1c[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla1c<-cbind(unitabla1c,SUITQUAL)
      rm(glc1992c)
      rm(SUITQUAL)
    }
    if(length(unitabla1w[,1])!=0){
      glc1992w<-raster("CTY41/GLC/glc1992_w.tif")
      SUITQUAL<-extract(glc1992w,unitabla1w[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla1w<-cbind(unitabla1w,SUITQUAL)
      rm(glc1992w)
      rm(SUITQUAL)
    }
    if(length(unitabla1c[,1])!=0&length(unitabla1w[,1])!=0){
      unitabla1<-rbind(unitabla1c,unitabla1w)
      rm(unitabla1c)
      rm(unitabla1w)
    }else{
      if(length(unitabla1c[,1])!=0&length(unitabla1w[,1])==0){
        unitabla1<-unitabla1c
      }
      if(length(unitabla1c[,1])==0&length(unitabla1w[,1])!=0){
        unitabla1<-unitabla1w
      }
    }
    rm(unitabla1c)
    rm(unitabla1w)
  ...
  if(any(GLC=="glc2000")){
    ...
    #Extraccion de uso de suelo en bruto
    unitabla2<-subset(unitabla,GLC=="glc2000")
    glc2000<-raster("CTY41/GLC/glc2000_300.tif")
    globlandc<-extract(glc2000,unitabla2[,c("DECLONGITUDE","DECLATITUDE")])
    unitabla2<-cbind(unitabla2,globlandc)
    rm(globlandc)
    rm(glc2000)
    
    #Extraccion de uso de suelo para cultivadas/silvestres ya interpretado
    unitabla2c<-subset(unitabla2,SAMPSTAT!=100&SAMPSTAT!=110&SAMPSTAT!=120|is.na(SAMPSTAT))
    unitabla2w<-subset(unitabla2,SAMPSTAT==100|SAMPSTAT==110|SAMPSTAT==120)
    if(length(unitabla2c[,1])!=0){
      glc2000c<-raster("CTY41/GLC/glc2000_c.tif")
      SUITQUAL<-extract(glc2000c,unitabla2c[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla2c<-cbind(unitabla2c,SUITQUAL)
      rm(glc2000c)
      rm(SUITQUAL)
    }
    if(length(unitabla2w[,1])!=0){
      glc2000w<-raster("CTY41/GLC/glc2000_w.tif")
      SUITQUAL<-extract(glc2000w,unitabla2w[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla2w<-cbind(unitabla2w,SUITQUAL)
      rm(glc2000w)
      rm(SUITQUAL)
    }
    if(length(unitabla2c[,1])!=0&length(unitabla2w[,1])!=0){
      unitabla2<-rbind(unitabla2c,unitabla2w)
      rm(unitabla2c)
      rm(unitabla2w)
    }else{
      if(length(unitabla2c[,1])!=0&length(unitabla2w[,1])==0){
        unitabla2<-unitabla2c
      }
      if(length(unitabla2c[,1])==0&length(unitabla2w[,1])!=0){
        unitabla2<-unitabla2w
      }
    }
    rm(unitabla2c)
    rm(unitabla2w)
  ...
  if(any(GLC=="glc2015")){
    ...
    #Extraccion de uso de suelo en bruto
    unitabla3<-subset(unitabla,GLC=="glc2015")
    glc2015<-raster("CTY41/GLC/glc2015_300.tif")
    globlandc<-extract(glc2015,unitabla3[,c("DECLONGITUDE","DECLATITUDE")])
    unitabla3<-cbind(unitabla3,globlandc)
    rm(globlandc)
    rm(glc2015)
    
    #Extraccion de uso de suelo para cultivadas/silvestres ya interpretado
    unitabla3c<-subset(unitabla3,SAMPSTAT!=100&SAMPSTAT!=110&SAMPSTAT!=120|is.na(SAMPSTAT))
    unitabla3w<-subset(unitabla3,SAMPSTAT==100|SAMPSTAT==110|SAMPSTAT==120)
    if(length(unitabla3c[,1])!=0){
      glc2015c<-raster("CTY41/GLC/glc2015_c.tif")
      SUITQUAL<-extract(glc2015c,unitabla3c[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla3c<-cbind(unitabla3c,SUITQUAL)
      rm(glc2015c)
      rm(SUITQUAL)
    }
    if(length(unitabla3w[,1])!=0){
      glc2015w<-raster("CTY41/GLC/glc2015_w.tif")
      SUITQUAL<-extract(glc2015w,unitabla3w[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla3w<-cbind(unitabla3w,SUITQUAL)
      rm(glc2015w)
      rm(SUITQUAL)
    }
    if(length(unitabla3c[,1])!=0&length(unitabla3w[,1])!=0){
      unitabla3<-rbind(unitabla3c,unitabla3w)
      rm(unitabla3c)
      rm(unitabla3w)
    }else{
      if(length(unitabla3c[,1])!=0&length(unitabla3w[,1])==0){
        unitabla3<-unitabla3c
      }
      if(length(unitabla3c[,1])==0&length(unitabla3w[,1])!=0){
        unitabla3<-unitabla3w
      }
    }
    rm(unitabla3c)
    rm(unitabla3w)
  ...
  if(any(GLC=="glc2022")){
    ...
    #Extraccion de uso de suelo en bruto
    unitabla4<-subset(unitabla,GLC=="glc2022")
    glc2022<-raster("CTY41/GLC/glc2022_300.tif")
    globlandc<-extract(glc2022,unitabla4[,c("DECLONGITUDE","DECLATITUDE")])
    unitabla4<-cbind(unitabla4,globlandc)
    rm(globlandc)
    rm(glc2022)
    
    #Extraccion de uso de suelo para cultivadas/silvestres ya interpretado
    unitabla4c<-subset(unitabla4,SAMPSTAT!=100&SAMPSTAT!=110&SAMPSTAT!=120|is.na(SAMPSTAT))
    unitabla4w<-subset(unitabla4,SAMPSTAT==100|SAMPSTAT==110|SAMPSTAT==120)
    if(length(unitabla4c[,1])!=0){
      glc2022c<-raster("CTY41/GLC/glc2022_c.tif")
      SUITQUAL<-extract(glc2022c,unitabla4c[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla4c<-cbind(unitabla4c,SUITQUAL)
      rm(glc2022c)
      rm(SUITQUAL)
    }
    if(length(unitabla4w[,1])!=0){
      glc2022w<-raster("CTY41/GLC/glc2022_w.tif")
      SUITQUAL<-extract(glc2022w,unitabla4w[,c("DECLONGITUDE","DECLATITUDE")])
      unitabla4w<-cbind(unitabla4w,SUITQUAL)
      rm(glc2022w)
      rm(SUITQUAL)
    }
    if(length(unitabla4c[,1])!=0&length(unitabla4w[,1])!=0){
      unitabla4<-rbind(unitabla4c,unitabla4w)
      rm(unitabla4c)
      rm(unitabla4w)
    }else{
      if(length(unitabla4c[,1])!=0&length(unitabla4w[,1])==0){
        unitabla4<-unitabla4c
      }
      if(length(unitabla4c[,1])==0&length(unitabla4w[,1])!=0){
        unitabla4<-unitabla4w
      }
    }
    rm(unitabla4c)
    rm(unitabla4w)
  ...
  if(all(GLC=="glc1992")){
    unitabla_a<-unitabla1
    rm(unitabla1)
  }
  if(all(GLC=="glc2000")){
    unitabla_a<-unitabla2
    rm(unitabla2)
  }
  if(all(GLC=="glc2015")){
    unitabla_a<-unitabla3
    rm(unitabla3)
  }
  if(all(GLC=="glc2022")){
    unitabla_a<-unitabla4
    rm(unitabla4)
  }
  #1234
  if(any(GLC=="glc1992")&any(GLC=="glc2000")&any(GLC=="glc2015")&any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla1,unitabla2,unitabla3,unitabla4)
    rm(unitabla1)
    rm(unitabla2)
    rm(unitabla3)
    rm(unitabla4)
  ...
  #123
  if(any(GLC=="glc1992")&any(GLC=="glc2000")&any(GLC=="glc2015")&!any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla1,unitabla2,unitabla3)
    rm(unitabla1)
    rm(unitabla2)
    rm(unitabla3)
  }
  #124
  if(any(GLC=="glc1992")&any(GLC=="glc2000")&!any(GLC=="glc2015")&any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla1,unitabla2,unitabla4)
    rm(unitabla1)
    rm(unitabla2)
    rm(unitabla4)
  }
  #234
  if(!any(GLC=="glc1992")&any(GLC=="glc2000")&any(GLC=="glc2015")&any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla2,unitabla3,unitabla4)
    rm(unitabla2)
    rm(unitabla3)
    rm(unitabla4)
  }
  #12
  if(any(GLC=="glc1992")&any(GLC=="glc2000")&!any(GLC=="glc2015")&!any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla1,unitabla2)
    rm(unitabla1)
    rm(unitabla2)
  }
  #13
  if(any(GLC=="glc1992")&!any(GLC=="glc2000")&any(GLC=="glc2015")&!any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla1,unitabla3)
    rm(unitabla1)
    rm(unitabla3)
  }
  #14
  if(any(GLC=="glc1992")&!any(GLC=="glc2000")&!any(GLC=="glc2015")&any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla1,unitabla4)
    rm(unitabla1)
    rm(unitabla4)
  }
  #23
  if(!any(GLC=="glc1992")&any(GLC=="glc2000")&any(GLC=="glc2015")&!any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla2,unitabla3)
    rm(unitabla2)
    rm(unitabla3)
  }
  #24
  if(!any(GLC=="glc1992")&any(GLC=="glc2000")&!any(GLC=="glc2015")&any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla2,unitabla4)
    rm(unitabla2)
    rm(unitabla4)
  }
  #34
  if(!any(GLC=="glc1992")&!any(GLC=="glc2000")&any(GLC=="glc2015")&any(GLC=="glc2022")){
    unitabla_a<-rbind(unitabla3,unitabla4)
    rm(unitabla3)
    rm(unitabla4)
  }
  #Verificacion final tabla unitabla_a es igual a Unitabla en num accesiones
  if(length(unitabla_a[,1])==length(unitabla[,1])){
    unitabla<-unitabla_a
    rm(unitabla_a)
  }else{
    stop("Number of accessions from original table and table after extract SUITQUAL values does not match")
  }
  
  #Correccion al alza de SUITQUAL para puntos que caen en cuerpos de agua de acuerdo uso de suelo pero en tierra de acuerdo a GADM
  for(i in 1:length(unitabla[,1])){
    unitabla$SUITQUAL[i]<-ifelse(unitabla$DISTOLAND[i]==0&unitabla$globlandc[i]==210,5,unitabla$SUITQUAL[i])
  }
  #Correccion al alza de SUITQUAL para puntos que caen en cuerpos de agua x GLC pero caen en anillo 1km o 10 km
  #para puntos a 1 km
  for(i in 1:length(unitabla[,1])){
    unitabla$SUITQUAL[i]<-ifelse(unitabla$DISTOLAND[i]==1&unitabla$globlandc[i]==210,10,unitabla$SUITQUAL[i])
  }
  
  #para puntos a 10 km
  for(i in 1:length(unitabla[,1]))
  {
    unitabla$SUITQUAL[i]<-ifelse(unitabla$DISTOLAND[i]==10&unitabla$globlandc[i]==210,5,unitabla$SUITQUAL[i])
  }
  write("SUITQUAL-Correcciones del parametro y fin del proceso", file="Error/process_info.txt", append=TRUE)
  
  #LOCALQUAL# -> A coordenadas en tierra, en mar
  ###########
  
  #Introduccion tabla GADM con descripcion de poligonos administrativos hasta NAME4
  
  if(all(GADM=="version2_8")){
    setwd(paste(ruta,"/CTY",sep=""))
    load("gadm2.RData")
    setwd(paste(ruta))
    #Asignacion de informacion GADM a las entradas a partir del codigo OBJECTID del poligono
    borrar<-merge(unitabla,gadm2,by="OBJECTID",all.x=T)
    unitabla1<-borrar[,c(-which(colnames(borrar)=="year"),-which(colnames(borrar)=="GADM"),-which(colnames(borrar)=="GLC"))]
    unitabla1<-subset(unitabla1,DISTOLAND==0)
    unitabla1<-unitabla1[,c(2:(ncol(unitabla1)-19),1,(ncol(unitabla1)-18):(ncol(unitabla1)))]
    write("17.LOCALQUAL-Introducci?n tabla GADM y asignaci?n de info GADM a partir del No de Pol?gono", file="Error/process_info.txt", append=TRUE)
    
    #introduccion tabla nivel maximo de comparacion ADM
    admlevel<-read.delim("ADMlevel.txt",encoding="latin1")
    admlevel<-admlevel[,c(1,4)]
    ...
    write("LOCALQUAL-Introducci?n tabla ADMlevel para niveles de comparaci?n admisibles por pa?s", file="Error/process_info.txt", append=TRUE)
  }
  if(!all(GADM=="version2_8")){
    if(any(GADM=="version2_8")){
      setwd(paste(ruta,"/CTY",sep=""))
      load("gadm2.RData")
      setwd(paste(ruta))
      borrar2<-subset(unitabla,unitabla$GADM=="version2_8")
      borrar2<-merge(borrar2,gadm2,by="OBJECTID",all.x=T)
      unitabla2<-borrar2[,c(-which(colnames(borrar2)=="year"),-which(colnames(borrar2)=="GADM"),-which(colnames(borrar2)=="GLC"))]
      unitabla2<-subset(unitabla2,DISTOLAND==0)
      unitabla2<-unitabla2[,c(2:(ncol(unitabla2)-19),1,(ncol(unitabla2)-18):(ncol(unitabla2)))]
      #introduccion tabla nivel maximo de comparacion ADM
      admlevel<-read.delim("ADMlevel.txt",encoding="latin1")
      admlevel<-admlevel[,c(1,4)]
      
      #Tabla temporal con informacion a comparar y nivel de comparacion
      local2<-unitabla2[,c(1,16,18,19,20,21,(ncol(unitabla2)-17),(ncol(unitabla2)-14),(ncol(unitabla2)-13),(ncol(unitabla2)-10),(ncol(unitabla2)-9),(ncol(unitabla2)-6),(ncol(unitabla2)-5),(ncol(unitabla2)-2),(ncol(unitabla2)-1))]
      local2<-merge(local2,admlevel,by="ORIGCTY",all.x=T)
      rm(unitabla2)
      rm(borrar2)
      rm(gadm2)
    }
    setwd(paste(ruta,"/CTY41",sep=""))
    load("gadm41.RData")
    setwd(paste(ruta))
    borrar3<-subset(unitabla,GADM=="version4_1")
    borrar3<-merge(borrar3,gadm2,by="OBJECTID",all.x=T)
    unitabla3<-borrar3[,c(-which(colnames(borrar3)=="year"),-which(colnames(borrar3)=="GADM"),-which(colnames(borrar3)=="GLC"))]
    unitabla3<-subset(unitabla3,DISTOLAND==0)
    unitabla3<-unitabla3[,c(2:(ncol(unitabla3)-19),1,(ncol(unitabla3)-18):(ncol(unitabla3)))]
    #introduccion tabla nivel maximo de comparacion ADM
    setwd(paste(ruta,"/CTY41",sep=""))
    admlevel<-read.delim("ADMlevel41.txt", encoding="latin1")
    admlevel<-admlevel[,c(1,4)]
    setwd(paste(ruta))
    #Tabla temporal con informacion a comparar y nivel de comparacion
    local3<-unitabla3[,c(1,16,18,19,20,21,(ncol(unitabla3)-17),(ncol(unitabla3)-14),(ncol(unitabla3)-13),(ncol(unitabla3)-10),(ncol(unitabla3)-9),(ncol(unitabla3)-6),(ncol(unitabla3)-5),(ncol(unitabla3)-2),(ncol(unitabla3)-1))]
    local3<-merge(local3,admlevel,by="ORIGCTY",all.x=T)
    rm(unitabla3)
    rm(borrar3)
    rm(gadm2)
    
    if(any(GADM=="version2_8")){
      local1<-rbind(local2,local3)
      rm(local2)
      rm(local3)
    }else{
      local1<-local3
    ...
    write("LOCALQUAL-Introduccion tabla ADMlevel para niveles de comparacion admisibles por pais", file="Error/process_info.txt", append=TRUE)
  }
  ##Comparacion entre cadenas de caracteres de descripcion de localidad
  ##proveniente de datos de pasaporte y provenientes de GADM a partir de coordenadas
  #funcion Agrep y distancia generalizada de Levenshtein
  
  #comparacion entre codigo ISO de pais
  comp1<-as.data.frame(matrix(nrow = length(local1[,1]), ncol = 41))
  for(i in 1:length(local1[,1]))
  {
    axb<- agrep(local1[i,1],local1[i,7],ignore.case=TRUE,value=F,max=list(all=0))
  ...
  write("LOCALQUAL-Finalizaci0n de los proceso de comparaci?n de t?rminos a determinados niveles seg?n nivel", file="Error/process_info.txt", append=TRUE)
  
  #Elaboracion tabla para revision visual sobre falsos positivos (aciertos con un margen de 2 deleciones/inserciones/cambios donde podria no haberlos)
  revisionFP<-subset(local1,ISOFP=="ORIGCTY/ISO iguales?"|ADM1FP=="ADM1/Name1-Varname1 iguales?"|ADM2FP=="ADM2/Name2-Varname2 iguales?"|ADM3FP=="ADM3/Name3-Varname3 iguales?"|ADM4FP=="ADM4/Name4-Varname4 iguales?")
  revisionFP<-revisionFP[,c(1,7,44,3,8,9,47,4,10,11,50,5,12,13,53,6,14,15,56)]
  #Exportar tabla para revisi?n en excel?
  #write.table(revisionFP, file = "Revision_Visual_Falsos_Positivos.txt", sep = "\t", col.names = NA, qmethod = "double")
  ...
  #Correccion caso Espanha, donde ADM3 no se usa en tablas de pasaporte, por lo que esa comparacion no debe tenerse en cuenta.
  #Para Espanha el maximo nivel de comparacion es ADM4, por lo que en caso de ocurrir 4 comparaciones positivas, el valor LOCALQUAL ser? 20
  for(i in 1:length(unitabla[,1]))
  {
    unitabla[i,(ncol(unitabla))]<- ifelse(unitabla[i,17]=="ESP"&unitabla[i,(ncol(unitabla))]==16,20,
                                          ifelse(unitabla[i,17]=="ESP"&unitabla[i,(ncol(unitabla))]==12,15,
  ...
  write("23.LOCALQUAL-Correcci?n caso Espa?a", file="Error/process_info.txt", append=TRUE)
  #anhadiendo 0 a valor final LOCALQUAL para puntos en mar
  for(i in 1:length(unitabla[,1]))
  {
    unitabla[i,(ncol(unitabla))]<- ifelse(is.na(unitabla[i,(ncol(unitabla))]),0,unitabla[i,(ncol(unitabla))])
  }
  ...
  #Determinacion de subparametros ERRORES, PRECIS, GEORBLE y INTERTEMP
  
  #Obtencion del anho de recoleccion INTERTEMP
  intertemp<-vector(mode="numeric",length = length(pasaporte[,1]))
  colldate<-as.numeric(substr(pasaporte$COLLDATE, 1,4))
  intertemp<-data.frame(pasaporte$ACCENUMB,colldate,intertemp)
  colnames(intertemp)[1]<-"ACCENUMB"
  ...
  #Uso de GEOREFMETH en caso de que haya datos disponibles. Solo aplicara si hay dato de GEOREFMETH para todos los casos
  nogeoref<-TRUE
  if(any(is.na(unitabla$GEOREFMETH))){
    nogeoref<-FALSE
  }
  ...
  write("31.Resultado2-Exportada tabla original m?s par?metros TOTALQUAL", file="Error/process_info.txt", append=TRUE)  
  #RESULTADO 3: Tabla UNITABLA con todos los parametros calculados (solo incluye entradas con coordenadas)
  
  
  colnames(unitabla)[which(colnames(unitabla)=="GEOREFMETH1")]<-"GEOREFM1"
  write.table(unitabla, file=paste(resultados,"/tabla_de_analisisGEOQUAL.txt",sep=""), sep = "\t", row.names = FALSE, qmethod = "double",fileEncoding = "latin1")
  write.table(unitabla, file=paste(resultados,"/tabla_de_analisisGEOQUAL.xls",sep=""), sep = "\t", row.names = FALSE, qmethod = "double",fileEncoding = "latin1")
  write("32.Resultado3-Exportada tabla con todos los par?metros y subpar?metros calculados m?s datos originales", file="Error/process_info.txt", append=TRUE)
  #RESULTADO 4: Tabla UNITABLA en formato shapefile
  
  geoqual<-SpatialPointsDataFrame(unitabla[,c(which(colnames(unitabla)=="DECLONGITUDE"),which(colnames(unitabla)=="DECLATITUDE"))],unitabla)
