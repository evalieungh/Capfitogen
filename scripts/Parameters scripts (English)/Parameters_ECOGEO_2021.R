...
pasaporte<-"PasaporteECU_GEOQUAL.txt"
#Text type parameter (text that goes between quotation marks "")
...
##### Note: This parameter =TRUE implies that it is going to be ecogeographically characterized, which will generate a folder with the particular results of said characterization

buffy<-FALSE #Only applies when ecogeo=TRUE
#TRUE or FALSE type parameter
...
##### Note2: selecting buffy=TRUE can slow down the extraction process, since it forces R to extract values for each point (that comes from the coordinates of the records in the passport table) not only from the cell where they fall (simple extraction, when buffy=FALSE) but of adjacent cells, whose values are averaged... that slows down the operation

tamp<-2000 #Only applies when buffy=TRUE
#Number type parameter (which will appear in blue in Rstudio)
...
resol1<-"Celdas 1x1 km aprox (30 arc-seg)" #Only applies when ecogeo=TRUE
...
##### Note2: ecogeographic (bioclimatic) variables for which you want to extract information for the points that come from the passport table:
...
##### Note2: ecogeographic (edaphic) variables for which you want to extract information for the points that come from the passport table
...
##### Note2: ecogeographic (geophysical) variables for which you want to extract information for the points that come from the passport table
...
##### Note2: si genotip=TRUE, the user must provide a specific table with the phenotypic characterization data. That table has a column in common with the passport table (ACCENUMB) and the rest are columns of variables called descriptors. This table, as in the case of the phenotypic characterization table, must be in the path CAPFITOGEN3/Pasaporte 

genot<-"AFLP.txt" #Only applies for genotip=TRUE
#Text type parameter (text that goes between quotation marks "")
##### Note1: where is the genotypic data and what are they called? (text box)
...
#csimilar<-8 #8=Sokal & Sneath 1963
#csimilar<-9 #9=Phi by Pearson
#csimilar<-10 #10=S2 by Gower & Legendre
...
#############################################
#Classic multivariate analysis: the parameters in this section can generate additional analyses and results to those generated by ecogeo, phenotip and genotip
#############################################
...
#Text type parameter (text that goes between quotation marks "")
##### Note1: Ecogeoclustype defines the multivariate clustering algorithm that will be used on the ecogeographic characterization data. The possibilities are:
...
#TRUE or FALSE type parameter
##### Note: Do you want a principal component analysis for all inputs by ecogeographic characterization data?

ecogeopcaxe<-5 #Only applies for ecogeopca=TRUE
#Number type parameter (which will appear in blue in Rstudio)
##### Note: ecogeopcaxe is a number that defines the principal components that will be analyzed in detail. The maximum number must be the number of ecogeographic variables used (bioclimv+edaphv+geophysv) -1. For example, if all the added variables of bioclimv+edaphv+geophysv = 20, ecogeopcaxe cannot be greater than 19.

phenoclus<-TRUE #Only applies for phenotip=TRUE
#TRUE or FALSE type parameter
##### Note: Do you want a single cluster analysis for all entries for the phenotypic characterization data?
...
#Text type parameter (text that goes between quotation marks "")
##### Note1: phenoclustype defines the multivariate clustering algorithm that will be used on the phenotypic characterization data. The possibilities are:
##### Note2: What type of hierarchical clustering do you want (UPGMA="average", single-linkage="single", complete-linkage="complete", Ward's method="ward" or weighted average="weighted")?

phenopca<-TRUE #Only applies for phenotip=TRUE
#TRUE or FALSE type parameter
##### Note1: Do you want a principal component/coordinate analysis for all inputs by ecogeographic characterization data?

phenopcaxe<-5 #Only applies for phenopca=TRUE
#Number type parameter (which will appear in blue in Rstudio)
##### Note1: Number of axes to extract (phenotypic analysis)
##### Note2: phenopcaxe is a number that defines the principal components that will be analyzed in detail. The maximum number must be the number of variables/descriptors that appear in the genotypic characterization table -1. If in that table, 15 columns with data (variables/descriptors) appear apart from the ACCENUMB column, then phenopcaxe cannot be greater than 14.
...
##### Note2: this is a necessary question to apply some methods in the script. To know if all the phenotypic variables/descriptors are quantitative or not, you must see in the table of the nature of the variables, if all the variables listed there appear as quantitative (phenovarq=TRUE) or if not all are quantitative (phenovarq=FALSE )
...
#TRUE or FALSE type parameter
##### Note1: Do you want to make matrix comparisons between the different components analyzed?
...
#Number type parameter (which will appear in blue in Rstudio)
##### note1: Number of permutations to use in the matrix correlations (numeric box)
##### note2: These numbers are usually between 100 and 10,000 or even more. However, a large number of permutations can cause memory errors or run times that are too long.
