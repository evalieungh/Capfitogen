...
write(paste("precision:", precision,sep=""), file="Parametros.Parameters.GEOQUAL.txt", append=TRUE)
...
for(i in 1:length(CTY))
{
  ...
  if(vvv<2015){
    paisext<-readShapePoly(paste(pais1),IDvar="OBJECTID",proj4string=CRS("+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"))
  }
  if(vvv>2015){
    paisext<-readOGR(paste(ruta,"/",pais1,".shp",sep=""))
...
#OPCION baja precisi?n
if(precision=="baja"){
  load("mapamarlr.RData")
  if(proj4string(enmar1)!=proj4string(mapamarlr)){
    proj4string(mapamarlr)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
    proj4string(enmar1)<-" +proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0"
  }
  DISTOLAND<-over(enmar1,mapamarlr)
  DISTOLAND2<-DISTOLAND$ERROR
  DISTOLAND2[is.na(DISTOLAND2)]<-30
  #Correcci?n de puntos en mar por GADM y dentro por WORLD lr
  DISTOLAND2[DISTOLAND2==0]<-1
  DISTOLAND<-data.frame(enmar[,1],DISTOLAND2)
  colnames(DISTOLAND)[1]<-"ACCENUMB"
  colnames(DISTOLAND)[2]<-"DISTOLAND"
  DISTOLAND<-merge(puntosorig,DISTOLAND,by="ACCENUMB",all.x=TRUE)
  DISTOLAND2<-DISTOLAND$DISTOLAND
  DISTOLAND2[is.na(DISTOLAND2)]<-0
  DISTOLAND<-cbind(DISTOLAND,DISTOLAND2)
  DISTOLAND<-DISTOLAND[,c(1,47)]
  colnames(DISTOLAND)[2]<-"DISTOLAND"
  write("12.Extracci?n de informaci?n para puntos que caen en mar_baja precisi?n", file="Error/process_info.txt", append=TRUE)
}
}
...
#Adici?n de COORQUAL a unitabla
...
writeOGR(geoqual,dsn=paste(resultados),layer="ShapefilePuntosGEOQUAL",driver="ESRI Shapefile")
...
writeOGR(geoqual["TOTALQUAL100"],paste(resultados,"/mapa_puntos_google.kml",sep=""), layer="TOTALQUAL", driver="KML")
