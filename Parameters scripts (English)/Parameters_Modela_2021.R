...
#Modela only has two parameters to complete in Rstudio. The rest of the parameters are filled in an excel table, which when ready is saved as a text (txt) file that must be located in the path X:/CAPFITOGEN/ModelOptions, and whose name must be defined in the partablename parameter
ruta<-"C:/CAPFITOGEN3"
...
#Text type parameter (text that goes between quotation marks "")
#### Note: this name corresponds to the name of the table (including the .txt extension) in text format, which contains all the parameters that configure the Modela tool
